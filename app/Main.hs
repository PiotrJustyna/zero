...
--          colour * (max 0 ((B4 0.0 1.0 0.0 1.0 :: B4 Float) `dot` ((B4 0.0 1.0 0.0 2.0 :: B4 Float) - vertex)))))
            let transformedPrimitiveStream :: PrimitiveStream Triangles (VertexFormat (B4 Float, B3 Float)) = (\(vertex, normal, colour) -> (mvpMatrix rX rY rZ tX tY tZ !* vertex, gpuMul colour (normal `dot` (light - vertex)))) <$> initialPrimitiveStream
