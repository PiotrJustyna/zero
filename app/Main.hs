...
main =
    runContextT defaultZeroContextFactory (ContextFormatColor RGB8) $ do
        vertexBuffer :: Buffer os (B4 Float, B3 Float) <- newBuffer (((numberOfTriangleVertices (head players)) * 3 * (length players)))
        uniformBuffer :: Buffer os (Uniform (B Float)) <- newBuffer 6
        let representationOfPlayers = (foldl (\acc x -> (asTriangles x) ++ acc) ([] :: [(V4 Float, V3 Float)]) players)
        writeBuffer vertexBuffer 0 representationOfPlayers
        let rawNormals1 = calculateRawNormalsForTriangles representationOfPlayers
        writeBuffer vertexBuffer ((numberOfTriangleVertices (head players)) * (length players)) (zipModelVerticesAndNormalVertices representationOfPlayers rawNormals1)
        shader :: CompiledShader os (ContextFormat RGBFloat ()) ((PrimitiveArray Triangles (B4 Float, B3 Float)), (PrimitiveArray Lines (B4 Float, B3 Float))) <- compileShader $ do
            initialPrimitiveStream :: PrimitiveStream Triangles (VertexFormat (B4 Float, B3 Float)) <- toPrimitiveStream (\x -> fst x)
...
loop :: Buffer os (B4 Float, B3 Float)
    -> CompiledShader os (ContextFormat RGBFloat ()) ((PrimitiveArray Triangles (B4 Float, B3 Float)), (PrimitiveArray Lines (B4 Float, B3 Float)))
...
loop vertexBuffer shader uniformBuffer transformations = do
    ...
    render $ do
        ...
        let primitiveArray :: PrimitiveArray Triangles (B4 Float, B3 Float) = toPrimitiveArray TriangleList (takeVertices ((numberOfTriangleVertices (head players)) * (length players)) vertexArray)
        let normalArray :: PrimitiveArray Lines (B4 Float, B3 Float) = toPrimitiveArray LineList (dropVertices ((numberOfTriangleVertices (head players)) * (length players)) vertexArray)
